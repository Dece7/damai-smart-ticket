...
class ChatService:
    ...
    async def chat(self, message: str, conversation_id: str | None, chat_type: str, user_id: int = None):
        """流式对话（支持 assistant / rag 两种模式）"""
        if not conversation_id:
            conv = memory_service.create_conversation(chat_type, user_id=user_id)
    ...
    async def _assistant_chat(self, message: str, conversation_id: int):
        ...
        for round_idx in range(MAX_TOOL_ROUNDS):
            ...
            for tool_call in response.tool_calls:
                ...
                yield {"type": "step", "step": step_num, "action": "reasoning", "content": f"选择工具: {tool_name}({args_str})"}

                tool_func = TOOLS_MAP.get(tool_name)
                if tool_func:
                    yield {"type": "step", "step": step_num, "action": "tool_start", "tool": tool_name, "content": f"执行: {tool_name}"}
        ...
        # 流式生成最终回答
        # 用 invoke 代替 astream，避免 bind_tools 影响流式输出
        # 然后手动分块推送，保持流式体验
        full_response = ""
        try:
            # 可能需要多轮：LLM 有时会再调一次工具才回答
            for _ in range(3):
                response = self.llm.invoke(messages)
                messages.append(response)

                # 如果 LLM 返回了工具调用，执行后再问一次
            ...
            full_response = "亲，抱歉暂时无法处理您的请求，请稍后再试~"
