# Phase 8：功能扩展与深度优化 — 知识点与面试要点

> Reranker 精排、离线评估体系、Function Calling 扩展、前端工程化的技术细节和面试准备。
...
| 指标 | 计算方式 | 说明 |
|------|----------|------|
| 来源命中率 @K | 答案文档是否在 Top-K 检索结果中 | 衡量检索质量 |
...
**面试问：** RAG 系统怎么评估？
**答：** 分层评估：① 检索质量（来源命中率 @K）— 检索结果是否包含正确文档；② 生成质量（关键词命中率）— 回答是否包含正确信息；③ 端到端（延迟）— 用户体验。先优化检索命中率，再优化生成质量。
...
**面试问：** 工具多了 LLM 怎么选择？
**答：** LLM 根据工具的 `name` 和 `description` 语义匹配用户意图。描述写得越清晰，选择越准确。最佳实践：① 工具名用动词短语（search_program）；② 描述包含适用场景和参数说明；③ 参数加类型注解和枚举约束。
