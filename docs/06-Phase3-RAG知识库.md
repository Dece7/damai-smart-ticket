...
> 文档加载、分块、Embedding、向量检索、RAG 问答链。
...
```
1. 文档加载
   docs/rag/*.md → TextLoader → Document 对象列表

2. 标题切分
   每篇文档 → MarkdownHeaderTextSplitter 按 #/##/### 切成 Parent 大块

3. 标题注入（元数据增强）
   每个 Parent 的 page_content 前面拼上 "h1 > h2\n"

4. Parent-Child 切分
   每个 Parent → RecursiveCharacterTextSplitter(400字) → 若干 Child 小块
   每个 Child 的 metadata 记录 parent_id

5. 保存 Parent
   parent_dict → data/parent_chunks.pkl（检索后回查用）

6. Embedding + 写入 ChromaDB
   Child chunks → DashScope embedding(1024维) → ChromaDB 持久化

7. 构建 BM25 索引
   Child chunks → 分词 → BM25Okapi → data/bm25_index.pkl
```

**注意**：BM25 索引和向量数据库是两套独立的检索系统。BM25 靠关键词匹配，向量靠语义相似。构建时分别构建，检索时混合使用（RRF 融合），互补覆盖更多场景。
...
```
第一步：按标题切出 Parent（完整章节，800 字）
  "## 退票政策\n亲，若显示不支持退...若显示条件退...大麦娱乐卡..."

第二步：对 Parent 二次切出 Child（语义集中，400 字）
  Child 1: "亲，若显示不支持退则不支持退票哦"
  Child 2: "若显示条件退则支持有条件退款"
  Child 3: "大麦娱乐卡退款后不支持再次购买"
```

检索时用 Child（精准），返回时用 Parent（完整）。一个 `##` 章节变成一个 Parent，一个 Parent 切出若干个 Child。

**面试要点：**
- 为什么要分块？LLM 的上下文窗口有限，不能把整个文档塞进去
- 分块太大 → 检索不精确；分块太小 → 丢失上下文
- Parent-Child 两步切分解决"精准检索 + 完整上下文"的矛盾
