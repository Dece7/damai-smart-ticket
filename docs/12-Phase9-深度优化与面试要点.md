# Phase 9：深度优化 — 知识点与面试要点

> 查询改写、多 Agent 协作、MCP 协议集成的技术细节和面试准备。
...
**集成位置：**
```
用户输入 → 查询改写 → BM25 + 向量检索 → RRF → Reranker → LLM 生成
            ↑
      新增环节
```

三种模式（Agent/贴心助手/规则助手）都生效。
...
**实际评估结果（HashEmbedding 基线）：**

| 指标 | 无改写 | 有改写 | 变化 |
|------|--------|--------|------|
| 来源命中率 @5 | 58.2% | 56.4% | -1.8% |
| 关键词命中率 | 54.5% | 47.3% | -7.2% |
| 平均延迟 | 77ms | 67ms | -10ms |

> HashEmbedding 精度低，改写效果被 Embedding 噪声掩盖。换真实 Embedding 模型后效果会更明显。

**面试要点：**
- 查询改写解决用户输入模糊导致检索命中率低的问题
- 用小模型（DeepSeek Flash）做改写，~100ms 延迟，不影响整体响应速度
...
| 问题 | 解决方案 |
|------|---------|
| 路由消息干扰子 Agent | ~~`_filter_route_messages()` 过滤 `__route__` 消息~~ → 已重构为结构化 state 路由，不再需要过滤 |
