...
class Conversation(Base):
    ...
    chat_type = Column(String(20), default="assistant")  # agent / multi_agent / assistant / rag
    pinned = Column(Integer, default=0)                   # 置顶标记
...
class Message(Base):
    ...
    steps = Column(JSON, nullable=True)      # 推理步骤（可视化回放）
    token_usage = Column(JSON, nullable=True) # Token 统计
    created_at = Column(DateTime)
```
...
- `JSON` 类型字段存储结构化数据（引用来源、推理步骤、Token 统计）
- `steps` 字段持久化推理过程，支持前端可视化回放
- `pinned` 字段支持对话置顶功能
...
每条 assistant 消息除了 content，还存储推理步骤和 Token 统计：
...
memory_service.save_message(
    ...
    role="assistant",
    content="周杰伦演唱会...",
    sources=[{"doc": "节目订票", "excerpt": "..."}],
    steps=[{"action": "tool_start", "content": "执行: search_program"}],
    token_usage={"prompt_tokens": 120, "completion_tokens": 85, "total_tokens": 205}
)
```

**面试要点：**
- 推理步骤持久化后，切换对话仍可查看历史推理过程
- Token 统计支撑管理员仪表盘（按天趋势、按模式对比）
- 四种模式都支持持久化（规则助手、贴心助手、Agent、多 Agent）
...
| 为什么用 SQLite 不用 MySQL？ | 轻量、无依赖、单文件部署，适合 Demo；生产环境换 MySQL |
| 推理步骤怎么持久化？ | 消息表 steps JSON 字段，存储推理过程，支持前端回放 |
| Token 统计有什么用？ | 成本监控、用量分析、支撑管理仪表盘统计 |
